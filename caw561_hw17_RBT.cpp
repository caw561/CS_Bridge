...
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) {
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        if(parent!=nullptr){
            ...
            if(parent->color == RED){
                if(parent->parent->left == parent){
                    curr_node = parent->parent->right;
                }else{
                    curr_node = parent->parent->left;
                }
                if(parent->parent!=nullptr&&parent->parent->color==RED){
                    swapColor(parent);
                    parent->left->color = RED;
                    parent->right->color = RED;
                }
                else if((curr_node!=nullptr && curr_node->color == BLACK) || curr_node == nullptr){
        ...
        if(point->left!=nullptr&&point->right!=nullptr&&point->color==BLACK && point->left->color == RED && point->right->color == RED){
